...
import client.input.basic.FloatInputParser;
import client.input.basic.StringInputParser;
...
public class PersonInputParser {
    ...
    public Person parsePerson() {
        ...
        while (true) {
            ...
            String weightInput = stringParser.parse("вес режиссера").trim();
            ...
            try {
                ...
                if (w <= 0) {
                    System.out.println("Вес должен быть > 0. Повторите ввод или оставьте пустым.");
                    continue;
                }
                person.setWeight(w);
                break;
            } catch (NumberFormatException e) {
                System.out.println("Неверный формат веса. Введите целое число > 0 или оставьте пустым.");
            }
        }
